...
def get_conviction_progression(
    conn: sqlite3.Connection, stake_ticks: List[int]
    ...
    for agent_id in sorted(all_agents):
        ...
        for proposal_id in sorted(all_proposals, key=int):
            ...
            if start_stake > 0 or end_stake > 0:
                ...
                if change > 0:
                    print(
                        f"  P{proposal_id}: {start_stake} → {end_stake} CP (+{change}) ✅"
                    )
                elif change < 0:
                    print(
                        f"  P{proposal_id}: {start_stake} → {end_stake} CP ({change}) ⚠️"
                    )
                else:
                    print(
                        f"  P{proposal_id}: {start_stake} → {end_stake} CP (no change)"
                    )
...
def analyze_switching_behavior(conn: sqlite3.Connection, stake_ticks: List[int]):
    ...
    for agent_name in sorted(agent_switches.keys()):
        ...
        successful_switches = [
            s for s in switches if s["event_type"] == "switch_recorded"
        ]
        ...
        if successful_switches:
            ...
            for i, switch in enumerate(successful_switches, 1):
                ...
                print(
                    f"   {i}. Tick {tick}: Moved {cp_amount} CP from P{source_pid} → P{target_pid}"
                )
                print(f"      Reason: {reason.replace('_', ' ').title()}")

                # Show conviction penalty
                print(
                    f"      💡 Conviction reset penalty: Lost built-up conviction multiplier on P{target_pid}"
                )
...
def analyze_detailed_staking_story(conn: sqlite3.Connection, stake_ticks: List[int]):
    ...
    for tick in sorted(events_by_tick.keys()):
        ...
        if stake_events or switch_events:
            ...
            for _, event_type, agent_id, payload_json, message in stake_events:
                ...
                stake_count += 1
                print(
                    f"   📈 {agent_name}: Staked {stake_amount} CP on P{proposal_id} ({choice_reason})"
                )
            ...
            for _, event_type, agent_id, payload_json, message in switch_events:
                ...
                switch_count += 1
                print(
                    f"   🔄 {agent_name}: SWITCHED {cp_amount} CP from P{source_pid} → P{target_pid} ({reason})"
                )
                print(
                    f"      ⚠️  Conviction penalty applied - lost multiplier progress on P{target_pid}"
                )
    ...
    if switch_count > 0:
        print(
            f"   • Strategic behavior: {switch_count}/{stake_count + switch_count} actions were switches ({switch_count/(stake_count + switch_count)*100:.1f}%)"
        )
        print(
            f"   • Agents demonstrated adaptive conviction management with penalty awareness"
        )
    else:
        print(
            f"   • All agents used traditional concurrent staking (no strategic switching)"
        )


def get_stake_ledger_progression(
    conn: sqlite3.Connection, all_ticks: List[int]
) -> Dict[int, List[Dict]]:
...
def calculate_original_stakes_from_ledger(
    stake_ledger: List[Dict],
...
def analyze_stake_ledger_progression(conn: sqlite3.Connection):
    ...
    print(
        f"   • PROPOSE/FEEDBACK/REVISE phases: ticks {all_ticks[0]} to {stake_ticks[0]-1 if stake_ticks else all_ticks[-1]}"
    )
    print(
        f"   • STAKE phase: ticks {stake_ticks[0]} to {stake_ticks[-1]}"
        if stake_ticks
        else "   • No STAKE phase found"
    )

    # Get stake ledger progression
    ledger_progression = get_stake_ledger_progression(conn, all_ticks)
    conviction_progression = get_conviction_progression(
        conn, stake_ticks if stake_ticks else []
    )
        ...
        if ledger:
            ...
            for record in ledger:
                ...
                if key not in initial_stakes:
                    ...
                    print(
                        f"     • {agent_id} → P{proposal_id}: {amount} CP (tick {tick_created})"
                    )
    ...
    if stake_ticks:
        ...
        for i, tick in enumerate(stake_ticks):
            ...
            print(
                f"      Ledger: {len(ledger)} records | Conviction: {len(conviction)} agents"
            )
                ...
                for agent_id, proposals in conviction.items():
                    ...
                    for proposal_id, conv_amount in proposals.items():
                        if conv_amount > 0:
                            ...
                            multiplier = (
                                conv_amount / orig_amount if orig_amount > 0 else 0
                            )
                            print(
                                f"        {agent_name} P{proposal_id}: {conv_amount} CP (×{multiplier:.2f})"
                            )
...
def create_detailed_conviction_matrix(conn: sqlite3.Connection, stake_ticks: List[int]):
    ...
    key_ticks = (
        [ticks[0], ticks[len(ticks) // 2], ticks[-1]] if len(ticks) >= 3 else ticks
    )
            ...
            for proposal_id in sorted_proposals:
                ...
                if total_conviction > 0 or original_stake > 0:
                    if original_stake > 0:
                        ...
                        print(
                            f"{agent_name:12}   P{proposal_id:8}   {original_stake:8}    ×{multiplier:7.2f}    {total_conviction:8}"
                        )
                    else:
                        print(
                            f"{agent_name:12}   P{proposal_id:8}   {original_stake:8}    × ?.??    {total_conviction:8}"
                        )
    ...
    for proposal_id in sorted_proposals:
        ...
        for agent_id in all_agents:
            start_conviction = (
                conviction_progression.get(start_tick, {})
                .get(agent_id, {})
                .get(proposal_id, 0)
            )
            end_conviction = (
                conviction_progression.get(end_tick, {})
                .get(agent_id, {})
                .get(proposal_id, 0)
            )
        ...
        if start_total > 0 or end_total > 0:
            print(
                f"P{proposal_id:8}   {start_total:8}   {end_total:8}   +{growth:5}    ×{avg_multiplier:7.2f}"
            )
...
def main():
    ...
    try:
        ...
        print(
            f"✅ Found STAKE phase: ticks {stake_ticks[0]} to {stake_ticks[-1]} ({len(stake_ticks)} ticks)"
        )
        ...
        switching_events = get_switching_events(conn, stake_ticks[0], stake_ticks[-1])
        successful_switches = len(
            [e for e in switching_events if e[1] == "switch_recorded"]
        )
        ...
        if successful_switches > 0:
            ...
            print(
                f"   • {successful_switches} successful switches moved CP without creating/destroying"
            )
            print(
                f"   • Agents showed strategic behavior: adapting positions based on traits"
            )
            print(f"   • Conviction penalty system intact: rounds reset on switching")
            print(
                f"   • Multiple switching motivations: strategic, feedback-based, hedging"
            )
            ...
            print(
                f"   • Switching infrastructure ready but not triggered by agent traits"
            )
