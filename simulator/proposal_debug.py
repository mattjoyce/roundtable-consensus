...
def load_state_from_db(db_path: str = "roundtable_state.db") -> Dict:
    ...
    try:
        ...
        state_data = {
            ...
            'agent_proposal_ids': json.loads(snapshot['agent_proposal_ids']) if snapshot['agent_proposal_ids'] else {},
            'proposals': json.loads(snapshot['proposals']) if snapshot['proposals'] else []
        }
...
def generate_proposal_debug_files(sim_id: str, issue_id: str, db_path: str = "roundtable_state.db"):
    ...
    try:
        ...
        if not proposal_weights and all_proposal_ids:
            ...
            proposal_weights = {pid: {'effective_weight': 1.0} for pid in all_proposal_ids}
            logger.info("No finalization weights found, using equal ranking for all proposals")
        ...
        if proposal_weights:
            ...
            ranked_proposals = [(pid, {'effective_weight': 0}) for pid in all_proposal_ids]
        
        # Create proposal_id to rank mapping (1-based ranking)
        proposal_ranks = {proposal_id: rank + 1 for rank, (proposal_id, _) in enumerate(ranked_proposals)}
        ...
        for agent_id, proposal_id in agent_proposal_ids.items():
            if proposal_id in proposals:
                ...
                agent_stakes = [
                    stake for stake in stake_ledger 
                    if stake.get('proposal_id') == proposal_id and stake.get('issue_id') == issue_id
                ]
                ...
                for stake in agent_stakes:
                    front_matter += f"  - tick: {stake.get('initial_tick', 'unknown')}\n"
                    front_matter += f"    amount: {stake.get('amount', 'unknown')}\n"
                    front_matter += f"    proposal_id: {stake.get('proposal_id', 'unknown')}\n"
                    front_matter += f"    stake_type: {stake.get('stake_type', 'unknown')}\n"
