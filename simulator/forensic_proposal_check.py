...
def check_agent_balances(cursor) -> bool:
    ...
    for agent_id in initial_balances:
        ...
        if agent_id in agents_with_proposal_burns:
            ...
            print(
                f"❌ {agent_id}: No proposal self-stake found (should be −{PROPOSAL_SELF_STAKE} CP)"
            )
            success = False
    ...
    if len(agents_with_proposal_burns) != len(initial_balances):
        print(
            f"❌ Mismatch: {len(agents_with_proposal_burns)} got burns, {len(initial_balances)} expected"
        )
        success = False
