...
def check_feedback_stakes(cursor) -> bool:
        ...
        if payload:
            try:
                ...
                if amount == FEEDBACK_STAKE:
                    ...
                    print(f"    ❌ Unexpected stake amount: {amount} (expected {FEEDBACK_STAKE})")
    ...
    if correct_stakes != len(feedback_burns):
        print(f"❌ Stake amount mismatch: {correct_stakes} correct out of {len(feedback_burns)}")
        return False
...
def check_feedback_limits(cursor) -> bool:
    ...
    for agent_id, feedback_count in agent_feedback_counts:
        if feedback_count > max_feedback_per_agent:
            print(f"❌ {agent_id}: {feedback_count} feedback (exceeds limit of {max_feedback_per_agent})")
            violations += 1
        else:
        ...
        try:
            ...
            if payload:
                ...
                if agent_proposal_id == target_proposal_id:
                    print(f"❌ T{tick} ({agent_id}) Self-feedback violation: feedback on own proposal #{target_proposal_id}")
...
def check_phase_lifecycle(cursor) -> bool:
    ...
    for tick, phase_tick, readiness_json in snapshots:
        ...
        try:
            ...
            print(f"  T{tick} Phase tick {phase_tick}: {ready_count}/{total_agents} agents ready")
        except json.JSONDecodeError:
