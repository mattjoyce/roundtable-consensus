...
class Phase:
    ...
    def _begin(
        self, state: RoundtableState, config: UnifiedConfig, creditmgr=None
    ) -> None:
    ...
    def is_complete(self, state: RoundtableState) -> bool:
        """Check if phase is complete and ready to advance."""
    ...
    def _begin(
        ...
        if state.current_issue:
            ...
            existing_noaction = next(
                (p for p in state.current_issue.proposals if p.proposal_id == 0), None
            )
...
class RevisePhase(Phase):
    ...
    def _do(
        ...
        for agent in agents:
            ...
            feedback_received = (
                []
            )  # Placeholder - should be populated with actual feedback
...
class FinalizePhase(Phase):
    ...
    def _execute_finalization(
        ...
        proposal_weights = self._aggregate_conviction_weights_inline(
            state, config, creditmgr
        )
        ...
        if not proposal_weights:
            log_event(
                LogEntry(
                    ...
                    payload={"issue_id": issue_id, "reason": "no_stakes_found"},
        ...
        winner_proposal_id, winner_data = self._determine_winner_inline(
            proposal_weights
        )

        # Emit finalization decision event
        self._emit_finalization_decision_inline(
            winner_proposal_id, winner_data, issue_id, tick, state
        )

        # Emit influence recorded events for winning proposal
        self._emit_influence_events_inline(
            winner_proposal_id, issue_id, tick, state, config, creditmgr
        )
    ...
    def _aggregate_conviction_weights_inline(
        self, state: RoundtableState, config: UnifiedConfig, creditmgr=None
    ):
        ...
        active_stakes = [
            stake
            for stake in state.stake_ledger
        ...
        for stake in active_stakes:
            ...
            growth_multiplier = creditmgr.calculate_growth_curve(
                time_held, conviction_params
            )
            ...
            if proposal_id not in proposal_weights:
                proposal_weights[proposal_id] = {
                    ...
                    "contributor_count": 0,
                    "first_stake_tick": (
                        stake.initial_tick if not stake.mandatory else first_stake_tick
                    ),
                }
    ...
    def _determine_winner_inline(self, proposal_weights):
        ...
        max_weight = max(
            data["total_effective_weight"] for data in proposal_weights.values()
        )

        # Get all proposals with max weight (for tie-breaking)
        tied_proposals = [
            (pid, data)
            for pid, data in proposal_weights.items()
            if data["total_effective_weight"] == max_weight
        ]

        # Tie-breaker: earliest first stake tick
        winner_proposal_id, winner_data = min(
            tied_proposals, key=lambda x: x[1]["first_stake_tick"]
        )

        return winner_proposal_id, winner_data

    def _emit_finalization_decision_inline(
        self, winner_proposal_id, winner_data, issue_id, tick, state
    ):
    ...
    def _emit_influence_events_inline(
        self, winner_proposal_id, issue_id, tick, state, config, creditmgr
    ):
...
def generate_phases(config: UnifiedConfig) -> List[Phase]:
    """Generate the sequence of phases for consensus execution."""
    phases = []
    phase_counter = 0
    ...
    phases.append(
        ProposePhase(
            phase_number=phase_counter, max_phase_ticks=config.propose_phase_ticks
        )
    )
    phase_counter += 1
    ...
    phases.append(
        FinalizePhase(
            phase_number=phase_counter, max_phase_ticks=config.finalize_phase_ticks
        )
    )
...
class Consensus:
    ...
    def tick(self):
        """Execute one tick of consensus progression with phase management."""
        self.state.tick += 1
        ...
        if current_phase.phase_type != self.state.current_phase:
            ...
            self.state.agent_readiness = {
                aid: False for aid in self.state.agent_readiness
            }
        else:
            self.state.phase_tick += 1
        ...
        log_event(
            LogEntry(
                ...
                phase=(
                    PhaseType(self.state.current_phase)
                    if self.state.current_phase
                    else None
                ),
                event_type=EventType.CONSENSUS_TICK,
                payload={"phase_tick": self.state.phase_tick},
        ...
        current_phase = self.get_current_phase()
        phase_ticks_expired = (
            current_phase and self.state.phase_tick > current_phase.max_phase_ticks
        )
        ...
        if self.all_agents_ready and phase_ticks_expired:
            log_event(
                LogEntry(
                    ...
                    event_type=EventType.PHASE_TRANSITION,
                    payload={"current_phase_index": self.current_phase_index},
    ...
    def get_current_phase(self) -> Phase:
        """Get the currently executing phase or None if complete."""
    ...
    def is_final_stake_round(self) -> bool:
        ...
        has_more_stake_phases = any(
            phase.phase_type == "STAKE" for phase in remaining_phases
        )
    ...
    def finalize_issue(self):
        ...
        # Emit finalization decision event
        self._emit_finalization_decision(
            winner_proposal_id, winner_data, issue_id, tick
        )
        ...
        self._print_finalization_summary(
            proposal_weights, winner_proposal_id, issue_id, tick
        )
    ...
    def _aggregate_conviction_weights(self):
        ...
        active_stakes = [
            stake
            for stake in self.state.stake_ledger
        ...
        for stake in active_stakes:
            ...
            # Calculate conviction multiplier for this stake
            growth_multiplier = self.creditmgr.calculate_growth_curve(
                time_held, conviction_params
            )
            effective_weight = round(stake.cp * growth_multiplier, 2)
    ...
    def _emit_finalization_decision(
        self, winner_proposal_id, winner_data, issue_id, tick
    ):
        """Emit the finalization decision event."""
        # Look up the proposal to get the author
        agent_id = "unknown"
    ...
    def _format_proposal_display(self, proposal_id, issue_id):
        ...
        if (
            bureau
            and bureau.current_issue
            and bureau.current_issue.issue_id == issue_id
        ):
            for proposal in bureau.current_issue.proposals:
    ...
    def _print_finalization_summary(
        self, proposal_weights, winner_proposal_id, issue_id, tick
    ):
        ...
        for rank, (proposal_id, data) in enumerate(ranked_proposals, 1):
            ...
            print(
                f"{marker:10} {formatted_proposal:<30} "
        ...
        for proposal_id, data in ranked_proposals:
            ...
            if agent_contributions:
                ...
                for (
                    ...
                    print(
                        f"   {agent_id:<20} {raw_stake:>6} CP × {multiplier:>5.3f} = {effective:>8.2f} CP  ({stake_count} stakes)"
                    )
            else:
    ...
    def _summarize_results(self):
        """Generate summary of consensus execution results."""
        phase_summary = []
